...
object ProfileManager {
    ...
    var allServers: MutableList<Profile> = arrayListOf()
    var smartServers: MutableList<Profile> = arrayListOf()
    var vpnState: BaseService.State = BaseService.State.Idle
    var isVpnConnected = {
        vpnState == BaseService.State.Connected
    }
    var isVpnStopped = {
    ...
    var isVpnStopping = {
        vpnState == BaseService.State.Stopping
    }
    var vpnConnectedTime: Long = 0
    var historyProfile: Profile? = null

    fun saveToHistoryProfile(): Profile {
        return (getCurrentProfileConfig()?: createProfile()).run {
            historyProfile = this
            this
        }
    }

    fun getCurrentProfileConfig() = ProfileManager.getProfile(DataStore.profileId)
    fun putSmartProfileConfig(): Profile? {
         ProfileManager.createProfile().also {
            val list = (smartServers.takeIf { !it.isNullOrEmpty() }
    ...
    fun createProfilesFromJson(jsons: Sequence<InputStream>, replace: Boolean = false) {
        ...
        jsons.asIterable().forEachTry { json ->
            Profile.parseJson(
                JsonStreamParser(json.bufferedReader()).asSequence().single(),
                feature
            ) {
                if (replace) {
                    lazyClear.value
    ...
    fun serializeToJson(profiles: List<Profile>? = getActiveProfiles()): JSONArray? {
        ...
        val lookup =
            LongSparseArray<Profile>(profiles.size).apply { profiles.forEach { put(it.id, it) } }
